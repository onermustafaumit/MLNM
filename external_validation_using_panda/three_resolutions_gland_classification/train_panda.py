...
from dataset_panda import Dataset
from tqdm import tqdm
from sklearn.metrics import confusion_matrix


parser = argparse.ArgumentParser(description='Train a multi-resolution gland classification model on PANDA dataset')
...
parser.add_argument('--slide_list_filename_train', default='../panda_dataset/panda__radboud__karolinska__all__train.txt', help='slide list train', dest='slide_list_filename_train')
parser.add_argument('--slide_list_filename_valid', default='../panda_dataset/panda__radboud__karolinska__all__valid.txt', help='slide list valid', dest='slide_list_filename_valid')
parser.add_argument('--slide_list_filename_test', default='../panda_dataset/panda__radboud__karolinska__all__test.txt', help='slide list test', dest='slide_list_filename_test')
...
parser.add_argument('--num_epochs', default=1000000, type=int, help='Number of epochs', dest='num_epochs')
parser.add_argument('--save_interval', default=500, type=int, help='Model save interval (default: 1000)', dest='save_interval')
...
print('slide_dir: {}'.format(FLAGS.slide_dir))
print('mask_dir: {}'.format(FLAGS.mask_dir))
...
# print('norm_layer: {}'.format(FLAGS.norm_layer))
...
# print('step_size: {}'.format(FLAGS.step_size))
...
train_dataset = Dataset(slide_dir=FLAGS.slide_dir, mask_dir=FLAGS.mask_dir, slide_list_filename=FLAGS.slide_list_filename_train, transforms=True)
num_slides_train = train_dataset.num_slides
print("Training Data - num_slides: {}".format(train_dataset.num_slides))

valid_dataset = Dataset(slide_dir=FLAGS.slide_dir, mask_dir=FLAGS.mask_dir, slide_list_filename=FLAGS.slide_list_filename_valid, transforms=False)
num_slides_valid = valid_dataset.num_slides
print("Validation Data - num_slides: {}".format(valid_dataset.num_slides))

test_dataset = Dataset(slide_dir=FLAGS.slide_dir, mask_dir=FLAGS.mask_dir, slide_list_filename=FLAGS.slide_list_filename_test, transforms=False)
num_slides_test = test_dataset.num_slides
print("Test Data - num_slides: {}".format(test_dataset.num_slides))

# define training and validation data loaders
data_loader_train = torch.utils.data.DataLoader(train_dataset, batch_size=FLAGS.batch_size, shuffle=True, num_workers=10)
data_loader_valid = torch.utils.data.DataLoader(valid_dataset, batch_size=FLAGS.batch_size, shuffle=False, num_workers=10)
data_loader_test = torch.utils.data.DataLoader(test_dataset, batch_size=FLAGS.batch_size, shuffle=False, num_workers=10)
...
with open(FLAGS.metrics_loss_file, 'w') as f:
    ...
    f.write('# slide_dir: {}\n'.format(FLAGS.slide_dir))
    f.write('# mask_dir: {}\n'.format(FLAGS.mask_dir))
    ...
    f.write('# epoch\tlearning_rate\ttraining_loss_high\ttraining_loss_medium\ttraining_loss_low\ttraining_loss_result\ttraining_loss_total \
            \tvalidation_loss_high\tvalidation_loss_medium\tvalidation_loss_low\tvalidation_loss_result\tvalidation_loss_total\n')
    
with open(FLAGS.metrics_acc_file, 'w') as f:        
    f.write('# epoch\tlearning_rate\ttraining_acc_high\ttraining_acc_medium\ttraining_acc_low\ttraining_acc_result\ttraining_acc_total \
            \tvalidation_acc_high\tvalidation_acc_medium\tvalidation_acc_low\tvalidation_acc_result\tvalidation_acc_total\n')
...
for epoch in range(FLAGS.num_epochs):
    ...
    for i, (img_high, img_medium, img_low, label) in enumerate(data_loader_train):
        ...
        img_high, img_medium, img_low, label = img_high.to(device), img_medium.to(device), img_low.to(device), label.to(device)
        output_high, output_medium, output_low, output_result = model(img_high, img_medium, img_low)
        output_total = output_high + output_medium + output_low + output_result    
        ...
        loss_total = loss_high + loss_medium + loss_low + loss_result
    ...
    # print('Epoch: {:d}, Learning rate: {:.8f}'.format(epoch + 1, lr_scheduler.get_last_lr()[0]))
    # print('Epoch: {:d}, Learning rate: {:.8f}'.format(epoch + 1, optimizer.param_groups[0]['lr']))
    print('Training loss high: {:.4f}\tTraining loss medium: {:.4f}\tTraining loss low: {:.4f}\tTraining loss result: {:.4f}\tTraining loss total: {:.4f}'.format(train_loss_high, train_loss_medium, train_loss_low, train_loss_result, train_loss_total))
    print('Training accuracy high: {:.4f}\tTraining accuracy medium: {:.4f}\tTraining accuracy low: {:.4f}\tTraining accuracy result: {:.4f}\tTraining accuracy total: {:.4f}'.format(train_acc_high, train_acc_medium, train_acc_low, train_acc_result, train_acc_total))
    ...
    with torch.no_grad():
        for i, (img_high, img_medium, img_low, label) in enumerate(data_loader_valid):
    ...
    print('Validation loss high: {:.4f}\tValidation loss medium: {:.4f}\tValidation loss low: {:.4f}\tValidation loss result: {:.4f}\tValidation loss total: {:.4f}' \
          .format(valid_loss_high, valid_loss_medium, valid_loss_low, valid_loss_result, valid_loss_total))
    print('Validation accuracy high: {:.4f}\tValidation accuracy medium: {:.4f}\tValidation accuracy low: {:.4f}\tValidation accuracy result: {:.4f}\tValidation accuracy total: {:.4f}' \
          .format(valid_acc_high, valid_acc_medium, valid_acc_low, valid_acc_result, valid_acc_total))
    # print('\n')
    

    
    with open(FLAGS.metrics_loss_file, 'a') as f:
        f.write('{:d}\t{:.8f}\t{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}\n' \
                .format(epoch + 1, optimizer.param_groups[0]['lr'], 
                        train_loss_high, train_loss_medium, train_loss_low, train_loss_result, train_loss_total, 
                        valid_loss_high, valid_loss_medium, valid_loss_low, valid_loss_result, valid_loss_total))

    with open(FLAGS.metrics_acc_file, 'a') as f:
        f.write('{:d}\t{:.8f}\t{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}\n' \
                .format(epoch + 1, optimizer.param_groups[0]['lr'], 
                        train_acc_high, train_acc_medium, train_acc_low, train_acc_result, train_acc_total, 
                        valid_acc_high, valid_acc_medium, valid_acc_low, valid_acc_result, valid_acc_total))

    with open(FLAGS.metrics_cm_file, 'a') as f:
        f.write('{:d}\t{:.8f}\t{:d}\t{:d}\t{:d}\t{:d}\t{:d}\t{:d}\t{:d}\t{:d}\t{:d}\t{:d}\t{:d}\t{:d}\t{:d}\t{:d}\t{:d}\t{:d}\t{:d}\t{:d}\t{:d}\t{:d}\t{:d}\t{:d}\t{:d}\t{:d}\t{:d}\t{:d}\t{:d}\t{:d}\t{:d}\t{:d}\t{:d}\t{:d}\t{:d}\t{:d}\t{:d}\t{:d}\t{:d}\t{:d}\t{:d}\t{:d}\n' \
                .format(epoch + 1, optimizer.param_groups[0]['lr'], 
...
with torch.no_grad():
    for i, (img_high, img_medium, img_low, label) in enumerate(data_loader_test):
...
# per-class accuracy
# class_acc_test_total = cm_test_total.diagonal() / cm_test_total.sum(1)

pbar.close()

with open(FLAGS.test_loss_file, 'w') as f:
    f.write('# test_loss_high\ttest_loss_medium\ttest_loss_low\ttest_loss_result\ttest_loss_total\n')
    f.write('{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}\n'.format(test_loss_high, test_loss_medium, test_loss_low, test_loss_result, test_loss_total))

with open(FLAGS.test_acc_file, 'w') as f:
    f.write('# test_acc_high\ttest_acc_medium\ttest_acc_low\ttest_acc_result\ttest_acc_total\n')
    f.write('{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}\n'.format(test_acc_high, test_acc_medium, test_acc_low, test_acc_result, test_acc_total))
...
with open(FLAGS.test_cm_file, 'w') as f:
    ...
    f.write('{:d}\t{:d}\t{:d}\t{:d}\t{:d}\t{:d}\t{:d}\t{:d}\t{:d}\t{:d}\t{:d}\t{:d}\t{:d}\t{:d}\t{:d}\t{:d}\t{:d}\t{:d}\t{:d}\t{:d}\n' \
